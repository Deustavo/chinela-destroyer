...
export const AudioManager = {
  getMusicLevel(): number {
    return Math.max(1, Math.min(10, parseInt(localStorage.getItem(KEY_MUSIC_LEVEL) ?? '2', 10)))
  },
  setMusicLevel(v: number): void {
    localStorage.setItem(KEY_MUSIC_LEVEL, String(Math.max(1, Math.min(10, v))))
  },
  getSfxLevel(): number {
    return Math.max(1, Math.min(10, parseInt(localStorage.getItem(KEY_SFX_LEVEL) ?? '2', 10)))
  },
